...
- Приложение должно быть написано преимущественно на SwiftUI.
- 1 экран должен быть реализован с использованием UIKit.
- Бизнес-логика приложения должна быть разделена от UI-логики; для SwiftUI кода необходимо использовать паттерн MVVM; для UIKit кода — MVP или MVVM.
- Бизнес-логика (сетевой слой, сервисы), по возможности, должна быть помещана в акторы;
- ViewModel классы должны быть `ObservableObject`; публичные свойства должны быть помечены аннотацией `@Published`.
- Передача зависимостей должна быть реализована с использованием `environmentObject()`.
- Необходимо отдавать предпочтение [структурированной многопоточности](https://docs.swift.org/swift-book/documentation/the-swift-programming-language/concurrency/) (`async let`, `withTaskGroup`) над использованием Task API.
- При удалении экрана из иерархии должны освобождаться все ресурсы, связанные с экраном (в том числе, отменяться создаваемые экраном (или его ViewModel) задачи - см `Task.cancel()`).
- Если используются сторонние библиотеки с `completion`-блоками, то необходимо написать обертки с `continuation` (используя, например, [`withCheckedContinuation`](https://developer.apple.com/documentation/swift/withcheckedcontinuation(function:_:))).
- iOS 15.0 и выше.
